 ...
 */
public class NodeTreeWalker implements NodeIterator, Iterator<Node>
{
    ...
    /**
     * Resets the current position in the tree,
     * such that calling <code>next()</code> will return the first Node again.
     */
    public void reset()
    {
    ...
    public Node next()
    {
        if (this.mNextNode != null)//check if we've already found the next Node by calling hasNext()
        {
