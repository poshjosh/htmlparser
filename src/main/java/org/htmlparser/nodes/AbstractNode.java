...
public abstract class AbstractNode implements Node, Serializable
{
     ...
     *      e.next().collectInto(collectionList, filter);
     * </pre>
    ...
    public Node getLastChild ()
    {
        ...
        if (numChildren == 0)
            return null;
        return children.get(numChildren - 1);
    }
    ...
    public Node getPreviousSibling ()
    {
        ...
        if (positionInParent < 1)//no previous siblings
            return null;
        return siblings.get(positionInParent - 1);
    }
    ...
    @Override
    public Node deepClone() throws CloneNotSupportedException {
      return deepClone(true, true);
    }

    @Override
    public Node deepClone(boolean parents, boolean children) throws CloneNotSupportedException {
      ...
      if (parents) {
        ...
        if (nodeParent == null) {
          ...
          cloneParent = nodeParent.deepClone(true, false);
        }
        ...
        if (nodeParent != null) {
          ...
          NodeList cloneSiblings = new org.htmlparser.util.NodeListImpl();

          for (Node nodeSibling : nodeSiblings) {

            Node cloneSibling;
            if (nodeSibling.equals(this)) {
              cloneSibling = clone;
            } else {
              cloneSibling = nodeSibling.deepClone(false, true);
            }
      ...
      if (children) {
        ...
        NodeList nodeChildren = this.getChildren();
        ...
        if (nodeChildren == null) {
          ...
          cloneChildren = new org.htmlparser.util.NodeListImpl();

          for (Node child : nodeChildren) {

            Node childClone = child.deepClone(false, true);
