 ...
 * <li>Tag</li>
 * </ul>
 * Each time <code>next()</code> is called, another node is returned until
        ...
        NodeFactory
{
    ...
    protected Consumer<Node> nodeConsumer;

    /**
     * Line number to trigger on.
     * This is tested on each <code>next()</code> call, as a debugging aid.
    ...
    public void setNodeConsumer(Consumer<Node> nodeConsumer) {
     ...
     * <code>next()</code> will return the first lexeme on the page.
     */
    public void reset ()
    {
        getPage ().reset ();
     ...
     * underlying page.
     */
    public Node next (boolean quotesmart) throws ParserException {
        
        final Node next = this.doNext(quotesmart);
    
        if(this.nodeConsumer != null) {
            
            this.nodeConsumer.accept(next);
        }
        
        return next;
    }
    
    private Node doNext (boolean quotesmart) throws ParserException {
        int start;
        char ch;
        Node ret;
     ...
     */
    protected Node makeTag (int start, int end, List<Attribute> attributes)
        throws
            ParserException
    {
        int length;
            ...
            ParserException
    {
        ...
        else
        {
            try
            {
                ...
                while (null != (node = lexer.next (false)))
