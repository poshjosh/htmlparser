 ...
 * {@link #visitAllNodesWith AbstractNodeVisitor}.
        ...
        VisitableNodes
{
    ...
    public Consumer<Node> getNodeConsumer() {
        return this.getLexer().getNodeConsumer();
    }

    public void setNodeConsumer(Consumer<Node> nodeConsumer) {
        this.getLexer().setNodeConsumer(nodeConsumer);
    }
    ...
    public NodeIterator elements (boolean recurse) throws ParserException{
        
        return (new IteratorImpl (getLexer (), getFeedback (), recurse));
    }
     ...
     * {
     *     Head head = heads.get (0); // there should be only one
    ...
    public HtmlDocument parse (NodeFilter filter) throws ParserException
    {
        ...
        ret = new HtmlDocumentImpl ();
        for (e = elements (); e.hasNext (); )
        {
            node = e.next ();
            if (null != filter)
    ...
    public void visitAllNodesWith (NodeVisitor visitor) throws ParserException
    {
        ...
        for (NodeIterator e = elements(); e.hasNext(); )
        {
            node = e.next();
            node.accept(visitor);
        }
